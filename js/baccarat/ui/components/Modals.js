...
export default class Modals {
    ...
    renderStats() {
        ...
        this.state.history.forEach(h => {
            if (h.patternList && h.patternList.length > 0) {
                h.patternList.forEach(p => {
                    if (p.win) {
                        totalHits++;
                        totalNet += (p.pred === 'B' && this.state.commissionExact) ? 0.95 : 1;
                    } else {
                        totalMisses++;
                        totalNet -= 1;
                    }
        ...
        const e = (id) => document.getElementById(id);
        if(e('kpi-hr')) { e('kpi-hr').innerText = hitRate + "%"; e('kpi-hr').className = `text-4xl font-black ${hitRate >= 50 ? 'text-[#30D158]' : 'text-[#FF453A]'}`; }
        if(e('kpi-signals')) e('kpi-signals').innerText = totalSignals;
        
        const netDisplay = (totalNet > 0 ? '+' : '') + (Number.isInteger(totalNet) ? totalNet : totalNet.toFixed(2));
        if(e('kpi-net')) { e('kpi-net').innerText = netDisplay; e('kpi-net').className = `text-2xl font-black ${totalNet >= 0 ? 'text-[#30D158]' : 'text-[#FF453A]'}`; }

        if(e('hud-w')) e('hud-w').innerText = totalHits;
        if(e('hud-h')) e('hud-h').innerText = totalSignals;
        if(e('hud-l')) e('hud-l').innerText = totalMisses;

        const trendTab = document.getElementById('tab-trend');
        if (trendTab && trendTab.getAttribute('data-active') === 'true') {
            if(window.app?.utils?.drawAdvancedGraph) window.app.utils.drawAdvancedGraph(bankroll, totalHits, totalMisses, 'graph-container', '#30D158', '#FF453A');
        }
    ...
    renderGoldenStats() {
        ...
        const e = (id) => document.getElementById(id);
        if(e('golden-kpi-hr')) { e('golden-kpi-hr').innerText = goldenHr + "%"; e('golden-kpi-hr').className = `text-xl font-black ${goldenHr >= 50 ? 'text-[#30D158]' : (totalGolden === 0 ? 'text-white' : 'text-[#FF453A]')}`; }

        const netDisplay = (goldenNet > 0 ? '+' : '') + (Number.isInteger(goldenNet) ? goldenNet : goldenNet.toFixed(2));
        if(e('golden-kpi-net')) { e('golden-kpi-net').innerText = netDisplay; e('golden-kpi-net').className = `text-xl font-black ${goldenNet >= 0 ? (goldenNet === 0 ? 'text-white' : 'text-[#FFD60A]') : 'text-[#FF453A]'}`; }

        if(e('golden-kpi-opps')) e('golden-kpi-opps').innerText = this.state.goldenBetsHistory.length;
        if(e('golden-kpi-dd')) e('golden-kpi-dd').innerText = (Number.isInteger(maxDrawdown) ? maxDrawdown : maxDrawdown.toFixed(2));

        const goldenTab = document.getElementById('tab-golden');
        if (goldenTab && goldenTab.getAttribute('data-active') === 'true') {
            if(window.app?.utils?.drawAdvancedGraph) window.app.utils.drawAdvancedGraph(goldenBankroll, goldenHits, goldenLosses, 'golden-graph-container', '#FFD60A', '#FF453A');
        }
    }
}
