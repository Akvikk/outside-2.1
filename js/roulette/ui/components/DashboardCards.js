...
export default class DashboardCards {
    ...
    renderDashboard() {
        ...
        this.state.pendingBets.forEach((bet, index) => {
            ...
            div.className = `grid-item p-2 pl-2.5 flex flex-col justify-between relative overflow-hidden select-none cursor-pointer ${styleClass} ${bet.confirmed ? 'card-confirmed' : ''}`;
            ...
            div.innerHTML = `
                <div class="w-full flex justify-between items-start relative z-10 gap-1">
                    <div class="flex flex-wrap items-center gap-1 flex-1 overflow-hidden opacity-90"><span class="text-[8px] uppercase font-bold tracking-widest opacity-80 bg-black bg-opacity-40 px-1 rounded truncate">${categoryLabel}</span></div>
