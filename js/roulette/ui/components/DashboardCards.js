...
export default class DashboardCards {
    ...
    renderDashboard() {
        ...
        this.state.pendingBets.forEach((bet, index) => {
            ...
            div.className = `grid-item animate-scale-in shrink-0 whitespace-normal relative overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent backdrop-blur-lg p-2 sm:p-2.5 flex flex-col justify-center gap-0.5 sm:gap-1 select-none cursor-pointer transition-transform hover:scale-105 hover:shadow-xl duration-300 ${styleClass} ${bet.confirmed ? 'card-confirmed' : ''} ${isHot ? 'ring-1 ring-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.25)]' : ''}${zeroHitClass}`;
