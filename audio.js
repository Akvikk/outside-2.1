export default class AudioController {
    constructor() {
        // Initialize AudioContext only once for the entire app
        this.audioCtx = (() => {
    ...
    playPrediction(enabled = true) {
        if (!enabled) return;
        this.playTone(880, 'sine', 0.1); this.playTone(1760, 'sine', 0.1, 0.05);
    }

    playWin(enabled = true) {
        if (!enabled) return;
        this.playTone(523.25, 'sine', 0.2, 0); this.playTone(659.25, 'sine', 0.2, 0.1); this.playTone(783.99, 'sine', 0.4, 0.2); this.playTone(1046.50, 'sine', 0.6, 0.3);
    }

    playLoss(enabled = true) {
        if (!enabled) return;
        this.playTone(150, 'sawtooth', 0.4); this.playTone(100, 'sawtooth', 0.4, 0.1);
    }
}