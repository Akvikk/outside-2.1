...
import { getStyle } from './PatternUtils.js';
...
export default class PatternScanner {
    ...
    scanPatterns(ignoreFilters = false, chasesObj = this.state.engineChases) {
        ...
        if (ignoreFilters || this.state.activeFilters.doz) {
            if (chasesObj['Dozens']) {
                alerts.push({ category: 'Dozens', patternName: 'FALSE BREAK', msg: `BET ${chasesObj['Dozens'].target}`, sub: `Chase Attempt ${4 - chasesObj['Dozens'].attemptsLeft}/3`, style: getStyle(chasesObj['Dozens'].target), targetToken: chasesObj['Dozens'].target });
            } else {
        ...
        if (ignoreFilters || this.state.activeFilters.col) {
            if (chasesObj['Columns']) {
                alerts.push({ category: 'Columns', patternName: 'FALSE BREAK', msg: `BET ${chasesObj['Columns'].target}`, sub: `Chase Attempt ${4 - chasesObj['Columns'].attemptsLeft}/3`, style: getStyle(chasesObj['Columns'].target), targetToken: chasesObj['Columns'].target });
            } else {
