export default class BankrollManager {
    ...
    static resolveUserBets(state, spinObj, eventBus) {
        ...
        state.pendingBets.filter(b => b.confirmed).forEach(bet => {
            ...
            state.confirmedBetLog.unshift({ betNumber: state.userStats.totalBets, pattern: bet.pattern, category: bet.category || 'Unknown', bet: bet.betName, resultSpin: spinObj.val, outcome: result });
        });
